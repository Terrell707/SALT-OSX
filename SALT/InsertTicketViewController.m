...
- (IBAction)submitBtn:(NSButton *)sender {
    ...
    // Otherwise, we will update the old ticket.
        ...
        if (updated) {
            NSLog(@"Ticket was successfully updated!");
            
            // Creates an array of witnesses that were removed from the ticket.
            NSMutableArray *oldWitnesses = [NSMutableArray arrayWithArray:[_oldTicket.helpedBy allObjects]];
            [oldWitnesses removeObjectsInArray:[newTicket.helpedBy allObjects]];
            // Creates an array of witnesses that were added to the ticket.
            NSMutableArray *newWitnesses = [NSMutableArray arrayWithArray:[newTicket.helpedBy allObjects]];
            [newWitnesses removeObjectsInArray:[_oldTicket.helpedBy allObjects]];
            
            // Removes all witnesses that no longer apply to the ticket.
            for (Expert *expert in oldWitnesses) {
                // Creates a new Witness object with this current expert and current ticket information. (Database Witnesses are
                //  automatically updated with the new ticket number.)
                Witness *oldWitness = [[Witness alloc] init];
                [oldWitness setExpert_id:expert.expert_id];
                [oldWitness setExpert:expert];
                [oldWitness setTicket_no:newTicket.ticket_no];
                [oldWitness setTicket:newTicket];
                updated = [[DataController sharedDataController] removeWitness:oldWitness];
                if (updated) {
                    NSLog(@"Witness %@ %@ was removed.", expert.first_name, expert.last_name);
                } else {
                    NSLog(@"Witness %@ %@ was not removed.", expert.first_name, expert.last_name);
                }
            }
            
            for (Expert *expert in newWitnesses) {
                // Creates a new Witness object with this current expert and ticket information. (Database Witnesses are
                //  automatically updated with the new ticket number.)
                Witness *newWitness = [[Witness alloc] init];
                [newWitness setExpert_id:expert.expert_id];
                [newWitness setExpert:expert];
                [newWitness setTicket_no:newTicket.ticket_no];
                [newWitness setTicket:newTicket];
                updated = [[DataController sharedDataController] insertWitness:newWitness];
                if (updated) {
                    NSLog(@"Witness %@ %@ was added.", expert.first_name, expert.last_name);
                } else {
                    NSLog(@"Witness %@ %@ was not added.", expert.first_name, expert.last_name);
                }
            }
